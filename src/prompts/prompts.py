...
def get_gpqa_web_thinker_instruction_memory(MAX_SEARCH_LIMIT=15):
    return """You are a reasoning assistant with the ability to perform web searches to help you answer the user's question accurately. You have special tools:
...
def get_deep_web_explorer_instruction_withurl(search_query, search_intent, search_result):
    return f"""You are a web explorer analyzing search results to find relevant information based on a given search query and search intent.
...
3. **Extract Relevant Information:**
- Return the relevant information from the **Searched Web Pages** that is relevant to the **Current Search Query**.
- Add corresponding URLs and titles to the mentioned important web pages.
...
def get_query_plan_instruction(question):
    return f"""You are a reasoning assistant. Your task is to generate a detailed query plan for answering the user's question by breaking it down into sub-queries. Each query should be a precise and suitable query for web search.
...
def get_query_plan_instruction_withurl(question):
    return f"""You are a reasoning assistant. Your task is to:
1. Generate a detailed query plan by breaking down and format the user's question into sub-queries, **each sub-query should be a precise and suitable query for web search**.
2. Extract any URLs mentioned in the user's question

Question: {question}

Please analyze the question carefully and:
- Break it down into multiple precise sub-queries suitable for web search, **each sub-query should be a precise and suitable query for web search**.
- Identify and extract any URLs mentioned (including partial URLs, domain names, or web addresses)
...
Note: If no URLs are found in the question, return an empty array for "urls".
"""
...
def get_multiqa_search_o1_instruction_memory(MAX_SEARCH_LIMIT):
    return (
        "You are a reasoning assistant with the ability to perform web searches to help "
        "you answer the user's question accurately. You have special tools:\n\n"
        "- To perform a search: write <|begin_search_query|> your query here <|end_search_query|>.\n"
...
def get_naive_rag_instruction_memory(question, documents):
    return (
...
def get_task_instruction_openqa_memory(question, current_memory, model_name=None):
    if model_name == 'qwq':
        user_prompt = (
            'Please answer the following question step by step. Your answer needs to include specific analysis and relevant citations (detail page url and title).\n\n'
            'Carefully understand the meaning of the question and the conditions involved in it.\n\n'
            'Current Memory:\n'
            f'{current_memory}\n\n'
            f'Question:\n{question}\n\n'
...
def get_task_instruction_openqa_memory_conclusion(question, current_memory, model_name=None):
    if model_name == 'qwq':
        user_prompt = (
            'Please answer the following question. Give your final answer in the format **Final Conclusion**\n'
            '[Your conclusion about the question]\n'
            'Current Memory:\n'
            f'{current_memory}\n\n'
            f'Question:\n{question}\n\n'
...
def get_task_instruction_multi_choice_memory(question, current_memory, model_name=None):
    if model_name == 'qwq':
        user_prompt = (
            'Please answer the following multiple-choice question. '
            'You should provide your final choice in the format \\boxed{YOUR_CHOICE}.\n\n'
